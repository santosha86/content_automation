...
- **Phase A add-on — open**: competitor scan in the Strategist (search YouTube for existing shorts on the picked story → skip saturated angles, feed the gap to the Hook Smith) *(adopted from Manus Wide Research)*
- **Phase B — in progress**: storyboard now renders end-to-end via `python -m pipeline.run --storyboard PATH` (adapter maps beats→segments + generates platform metadata). Still to craft: `must_show` proof b-roll relevance, FLUX local image gen for `generated_image` beats, split-screen/zoom layouts, background music by mood, branded end-card (the three Gate B gaps a storyboard render currently flags), genre pacing presets — storyboard mood → cut rhythm + camera energy *(adopted from Higgsfield Cinema Studio)*
- **Phase C**: publisher — YouTube Data API + Instagram Graph API, scheduled daily run; repurposing station — every storyboard also drafts a LinkedIn post + X thread via Typefully *(adopted from Manus multi-format)*
- **Phase D**: your cloned voice, local-only (Chatterbox, per-emotion reference clips); avatar evaluation; trained visual identity for any recurring presenter/mascot — local FLUX LoRA from reference photos *(adopted from Higgsfield Soul ID)*
- **Phase E**: analytics loop — Analyst agent (Fable 5) reads YT/IG metrics, proposes `style_guide.md` edits, A/B tests hooks; channel memory — covered topics/claims/entities store for follow-ups, callbacks, and no repeats *(adopted from AirOps Knowledge Base)*
